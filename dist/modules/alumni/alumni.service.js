"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.AlumniService = void 0;
const common_1 = require("@nestjs/common");
const mongoose_1 = require("@nestjs/mongoose");
const mongoose_2 = require("mongoose");
const alumni_schema_1 = require("./entities/alumni.schema");
let AlumniService = class AlumniService {
    constructor(alumniModel) {
        this.alumniModel = alumniModel;
    }
    async findAll() {
        return this.alumniModel.find().exec();
    }
    async findOne(id) {
        const alumni = await this.alumniModel.findById(id).exec();
        if (!alumni)
            throw new common_1.NotFoundException(`Alumni with _id ${id} not found`);
        return alumni;
    }
    async create(createAlumniDto) {
        const newAlumni = new this.alumniModel(createAlumniDto);
        return newAlumni.save();
    }
    async update(id, updateAlumniDto) {
        const updated = await this.alumniModel
            .findByIdAndUpdate(id, updateAlumniDto, { new: true })
            .exec();
        if (!updated)
            throw new common_1.NotFoundException(`Alumni with _id ${id} not found`);
        return updated;
    }
    async remove(id) {
        const result = await this.alumniModel.findByIdAndDelete(id).exec();
        if (!result)
            throw new common_1.NotFoundException(`Alumni with _id ${id} not found`);
    }
};
exports.AlumniService = AlumniService;
exports.AlumniService = AlumniService = __decorate([
    (0, common_1.Injectable)(),
    __param(0, (0, mongoose_1.InjectModel)(alumni_schema_1.Alumni.name)),
    __metadata("design:paramtypes", [mongoose_2.Model])
], AlumniService);
//# sourceMappingURL=alumni.service.js.map